...
def divide_if_identical_col(path, id=0):
    ...
    for k, g in groupby(enumerate(path), lambda i_x: i_x[1][id].col):
...
def divide_if_identical_row(path, id=0):
    ...
    for k, g in groupby(enumerate(path), lambda i_x1: i_x1[1][id].row):
        groups.append(list(map(itemgetter(1), g)))
...
def divide_if_discontinuous_col(path, id=0):
    ...
    if id == 0:
        path.sort(key=lambda x: x[id].col)
    for k, g in groupby(enumerate(path), lambda i_x2: i_x2[0] - i_x2[1][id].col):
        groups.append(list(map(itemgetter(1), g)))
...
def divide_if_discontinuous_row(path, c_id=0):
    ...
    for k, g in groupby(enumerate(path), lambda i_x3: i_x3[0] - i_x3[1][c_id].row):
        groups.append(list(map(itemgetter(1), g)))
...
def cluster_by_columns(path, i=0, continuous=False, identical_row=False):
    ...
    if continuous:
        for group in list(cluster_c.values()):
...
def cluster_by_rows(path, i=0, continuous=False, identical_row=False):
    ...
    for tran in path:
        if tran[i]:
            if tran[i].row not in list(cluster_r.keys()):
    ...
    if continuous:
        for group in list(cluster_r.values()):
